...
//      debug(A);
      compare_cnt = 0;
      Check(0, n - 1) || quick_sort(0, n - 1);
      if (compare_cnt == 61) {
        debug(O);
      }
      max_compare = max(max_compare, compare_cnt);
    } while (next_permutation(O.begin(), O.end()));
    debug(max_compare, clock() * 1.0 / CLOCKS_PER_SEC);
  }
  return 0;
}
